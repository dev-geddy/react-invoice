...
- `e2e/invoices.spec.ts` — shared ledger: owner creates an invoice (2 × 100 at 21% → 242 payable, listed), a teammate reads another user's invoice read-only, lock/unlock freezes and releases the form. Short numeric labels need `{ exact: true }` ("Rate" also matches "VAT rate %"). Satisfies `L2-INVOICE-18/19`. Also: a teammate is bounced from `/backflip/invoicing/settings`, and a series in use cannot be removed (`L2-INVOICE-23/24`).
