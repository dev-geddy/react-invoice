...
- `apps/web/app/getting-started/intro/page.tsx` — discovery page (RSC, static): what React Invoice is, `IN_THE_BOX` + `AUDIENCE` card grids, CTA → setup wizard. Satisfies `L2-UI-18`.
...
- Mount points: public `site-footer.tsx` (baseline-aligned after `© <year> React Invoice`) and admin Overview `page.tsx` (last child of the page column, centered). Nothing else mounts it.
