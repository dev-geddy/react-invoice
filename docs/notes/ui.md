...
- `apps/web/app/page.tsx` — public marketing homepage (RSC/SSR). Composes `_components/`: `SiteHeader`, `Hero`, `SellingPoints`, `DocumentSection`, `PlatformGrid`, `HowItWorks`, `CtaBand`, `WordmarkBand`, `SiteFooter`, plus `PaperFloatKeyframes`. Sets page `metadata`. Rebuilt from the Claude Design canvas `Homepage.dc.html` (project `7acd0e2d-3009-4928-ba8f-72ddc21a01a2`), which pitches the invoicing product first and Backflip as what it stands on.
- **Brand accent**: the homepage owns a red accent the admin console does not — `--brand`, `--brand-soft`, `--brand-line`, `--brand-ink`, `--brand-stripe`, `--brand-paper`, `--paper-shadow`, declared as arbitrary properties on the page root in `page.tsx` (`BRAND_TOKENS`) with a `dark:` set beside each (lighter, less saturated — the light-mode lightness goes muddy on near-black). Deliberately **not** added to `packages/ui/globals.css`: nothing under `/backflip` should inherit it. `SiteHeader`/`SiteFooter` are shared with `/getting-started/*`, which declares no brand tokens, so both read them as `var(--brand, var(--primary))` and fall back to the neutral theme there.
- `apps/web/app/_components/*` — homepage sections (app-scoped, `L1-ARCH-07/08`): `site-header.tsx` (`"use client"` — sticky nav + wordmark + `ThemeToggle`; Platform anchor (`/#platform`, `sm:` and up), Getting started, then a brand-filled Admin link), `theme-toggle.tsx` (dark/light switch via `useTheme`, mount-guarded; shared by public and admin headers), `brand-icon.tsx` (shared arc glyph SVG — public wordmark and admin sidebar tile), `hero.tsx` (badge, headline, three CTAs, a mono fact strip with trailing `/` separators, GitHub link, and `InvoicePaperStack` beside it), `invoice-paper.tsx` (decorative floating invoice: two blank sheets behind one rendered document, sample figures, whole stack `aria-hidden` and `lg:` only), `selling-points.tsx` (5 numbered reasons on `bg-muted`), `document-section.tsx` ("the preview is the document" — 4 check points, the PDF filename chip, and a floating ledger card), `platform-grid.tsx` (`id="platform"` — 6 Backflip baseline `Card`s on `bg-muted`), `how-it-works.tsx` (3 steps, cards topped with a brand rule), `cta-band.tsx` (closing band on `--brand-soft`), `paper-float.tsx` (the three float keyframes + a `prefers-reduced-motion` kill switch, mounted once by the page), `wordmark-band.tsx` (oversized brand-tinted wordmark), `site-footer.tsx` (nav now carries GitHub; copyright reads `· Source-available` and carries `AppVersion`), `app-version.tsx`. Theme tokens + the page's brand tokens only, no hex. Icons remixicon only.
...
- `apps/web/app/getting-started/intro/page.tsx` — discovery page (RSC, static): what Backflip Invoice is, `IN_THE_BOX` + `AUDIENCE` card grids, CTA → setup wizard. Satisfies `L2-UI-18`.
...
- Screenshot: `docs/assets/admin-integrations.png` (1200×600, unconfigured state, demo seed user). Used here only — the repo `README.md` no longer embeds it. Regenerate with `.screenshots/shoot.mjs` (gitignored local Playwright driver) and re-copy.
...
- Storage namespaces differ (`react-invoice.setup` vs `react-invoice.setup.docker`) so both guides can be open in one tab without mixing values.
...
- `-n <app-name>` / `--app-port <port>` are appended **only** when the entered value differs from the script defaults (`react-invoice` / `3080`).
...
- Step 5 (database) states the overlap: provisioning already installed Docker, so `setup-droplet-db-docker.sh` is the "adding a dockerized db to a droplet provisioned another way" path. The container itself starts on the first deploy; data lives in the named volume `react_invoice_pgdata`.
...
## Homepage sections — the redesign
Replaced `capability-list.tsx`, `feature-list.tsx`, `numbered-list.tsx` and `claude-connector.tsx` (all deleted; no L2 clause referenced them).
- **Why**: the old page pitched the *foundation* — two five-item lists (non-developer / developer views of the same slots) plus an MCP connector section. The design canvas pitches the *product*: invoicing you own, then Backflip underneath it as the reason it is extensible. The connector is a `/backflip` feature, not a reason to fork, so it left the homepage; `docs/notes/mcp.md` remains its home.
- `selling-points.tsx` keeps the numbered-row layout the deleted `numbered-list.tsx` established (large low-contrast index · icon + title · body; `md` grid `[4rem, 1fr, 1.15fr]`, folding below that). The index is `aria-hidden` — visual rhythm, not real order — and titles stay `<h3>`. Rows beat cards here for the same reason as before: five cards wrap 4 + 1 and orphan the fifth.
- **Every claim maps to something that ships** — checked against `L2-AUTH` roles, `L2-INVOICE` (series snapshots, VAT rule, PDF naming), the integrations pages and the repo LICENSE. Not a roadmap section; keep it that way.
- Bands alternate: hero → `bg-muted` selling points → plain document section → `bg-muted` platform grid → plain how-it-works → `--brand-soft` CTA → wordmark.
- **Motion**: three `paper-float-*` keyframes drift the decorative sheets out of phase. `paper-float.tsx` also kills every one of them under `prefers-reduced-motion` (`[class*="paper-float"]`), which is why the animation utilities must keep that literal in their class name.
- **Gotcha (fixed)**: the document section's text column needs `min-w-0`. The PDF filename chip is `whitespace-nowrap`, and a grid item's default `min-width:auto` let it set the column width — 193px of page-level horizontal scroll at 390px wide, until `min-w-0` capped it (`L2-UI-16`).
- Verified after the rebuild: no horizontal scroll at 390/1440, zero console errors or warnings, light + dark checked (`L2-UI-16`, `L2-UI-17`).
...
- Mount points: public `site-footer.tsx` (baseline-aligned after `© <year> Backflip Invoice`) and admin Overview `page.tsx` (last child of the page column, centered). Nothing else mounts it.
...
## Admin nav
- Sidebar groups: Platform (Overview, UI samples), **Invoicing** (Invoices, Customers, Series & currency), Settings (Members, Account, Integrations, pinned bottom). Active item is resolved by longest-prefix match over every nav url, so `/backflip/invoicing/customers` highlights Customers rather than Invoices (`L2-INVOICE-30`).
