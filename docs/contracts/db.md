...
- `L2-DB-33` — `invoice_series` table: `id`, `code` (unique), `currency` (default `€`, added in migration `0015`), `brandName`, `brandSubName`, `createdAt`, `updatedAt`. Migration `0013` creates. No FK from `invoice` — invoices store the code and branding as a snapshot (`L2-INVOICE-22`). Owned by the `invoice` domain.
...
- `L2-DB-35` — `invoice_config` table (single row, `kind` unique default `invoice`): `id`, `kind`, `brandName`, `brandSubName`, `taxYearStartMonth` (default 4), `taxYearStartDay` (default 6, both added in migration `0017`), `updatedAt`. Migration `0016` creates; no seed — read-or-create on first save, like the other config singletons. Owned by the `invoice` domain (`L2-INVOICE-32`).
