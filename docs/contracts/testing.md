...
- `L2-TEST-02` — `corepack yarn workspace web test:e2e` → Playwright chromium, specs in `apps/web/e2e/`. `webServer` invokes `next dev -p 3180` directly (bypasses dotenv-cli) with explicit env: test `DATABASE_URL`, fixed `AUTH_SECRET`, `NEXT_DIST_DIR=.next-e2e`, no `AUTH_GOOGLE_*` (credentials-only). Config `apps/web/playwright.config.ts`.
- `L2-TEST-03` — e2e `globalSetup` (`apps/web/e2e/global-setup.ts`): create `react_invoice_test` on `localhost:${POSTGRES_PORT:-5545}` if missing, run drizzle migrations, truncate auth tables, reseed fixtures `owner@e2e.test` (owner) + `teammate@e2e.test` (teammate) with bcrypt hashes. Never drops the DB (live pools under `reuseExistingServer`).
...
## Invariants
- `L2-TEST-04` — e2e never reads or mutates the dev `react_invoice` database; suite runs only against `react_invoice_test`.
- `L2-TEST-05` — e2e dev server never shares `.next` with the 3080 dev server: builds into `.next-e2e` via `NEXT_DIST_DIR` (honored in `next.config.ts`).
...
## Errors
- `L2-TEST-07` — `react-invoice-db` container down → e2e globalSetup fails at connect. Fix: `docker compose up -d`.

## Acceptance
- `L2-TEST-08` — With `react-invoice-db` up: unit suite green, e2e suite green, dev `react_invoice` database row set unchanged after the run.
