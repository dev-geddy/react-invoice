...
- `L2-INVOICE-32` — Branding falls back: a series with an empty `brandName`/`brandSubName` prints the platform brand from `invoice_config` (edited in the Brand block of Series & currency). The fallback is resolved server-side, so the invoice snapshots the brand it actually prints.
