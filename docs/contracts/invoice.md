...
- `L2-INVOICE-22` — `invoice_series` table: `id`, `code` (unique — the numbering prefix), `brandName`, `brandSubName`, `createdAt`, `updatedAt`. See `L2-DB-33`. An invoice **snapshots** the code and both brand parts when saved, so editing or deleting a series never rewrites issued invoices.
