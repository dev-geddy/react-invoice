 ...
 * `react_invoice_test` database, then `webServer` boots the app against it with an
 ...
 *
 * Requires the local postgres container (`react-invoice-db`, port 5545) to be up.
 *
...
export default defineConfig({
  ...
  webServer: {
    ...
    env: {
      ...
      // issuerOrigin()'s unset-AUTH_URL fallback is the *dev* port (3080), not
      // this suite's PORT_APP (3180) — set it explicitly so the OAuth issuer,
      ...
      // Own build dir so a dev server on 3080 and the e2e server never share `.next`.
